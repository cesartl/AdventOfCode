...
//doNotGetCaught(state, 0)
//def lol(n: Int) = doNotGetCaught(parse(cesar).delay(n), n)

//lol(221)

def findDelay(state: State) = {
  var delay = 1
  while (!state.layers.forall(p => (delay + p._1) % (2 * (p._2.depth-1)) != 0)) {
    delay += 1
  }
  delay
}

def score2(state: State) = state.layers.filter(p => p._1 % (2 * (p._2.depth-1)) == 0).map(p => p._1 * p._2.depth).sum

score2(state)

score2(parse(cesar))
score(advance(parse(cesar), Map())._2)
