...
val l = List("<{!>}>", "<!!>", "<!!!>>", "<{o\"i!a,<{i<a>", "{{<!>},{<!>},{<!>},{<a>}}", "{{<a>},{<a>},{<a>},{<a>}}")

l.map(s => s.replaceAll("!(.)", ""))
...
case class EmptyGroup() extends Tree {
  override def children() = List()
}

case class SingleGroup(group: Tree) extends Tree {
  override def children() = List(group)
}

case class Group(groups: Seq[Tree]) extends Tree {
  override def children() = groups
}

val garbageRegex = "<(.*?)>"
val groupRegex = Pattern.compile("\\{(.*)\\}")

def clean(input: String) = {
  val str = input.replaceAll("!(.)", "")
  println(str)
  str.replaceAll(garbageRegex, "_")
}


groupRegex.matcher("{{<ab>},{<ab>},{<ab>},{<ab>}}").matches()

val buf = scala.collection.mutable.ArrayBuffer.empty[String]

def findNextComma(input: String): Int = {
  if(input.isEmpty){
    return -1
  }
  var open = 0
  var current = input(0)
  var i = 0
  while (!(current.equals(',') && open == 0) && i < input.length) {
    current = input(i)
    //    println("current " + current + " open " +open )
    if (current.equals('{')) {
      open += 1
...
def parseList(input: String): List[String] = {
  val buf = scala.collection.mutable.ArrayBuffer.empty[String]
  var start = 0
  var end = findNextComma(input)
  if (end == -1) {
    return List(input)
  }
  while (end > start) {
    //    println("start: "+start)
    //    println("end: " + end)
    buf += input.substring(start, end)
    start = end + 1
    end = start + findNextComma(input.substring(start))
  }
  buf += input.substring(start, input.length)
  buf.toList
}
...
def parse(input: String, ident: String): Tree = {
  buf += (ident + "doing " + input)
  if (input.equals("{}") || input.matches("\\{_*\\}")) EmptyGroup() else {
    if (input.length == 1) {
      buf.foreach(println)
      throw new IllegalArgumentException(input)
    }
    val inner = input.substring(1, input.length - 1)
    Group(parseList(inner).map(s => s.trim()).filter(s => !s.equals("") && !s.equals("_")).map(c => parse(c, ident + "\t")).toList)
  }
}
